...
1. 一个 AI Relay Box 桌面应用安装包，或者本地开发环境
2. 至少一个可用的上游 Provider / 中转服务
3. 一个支持自定义 API endpoint 的工具

## 第一步：启动 AI Relay Box

启动桌面应用后，AI Relay Box 会在本地启动一个 API Gateway。
...
这里的本地 API Key 通常可以是任意非空字符串，因为真正的上游密钥由 AI Relay Box 在本地转发时处理。
